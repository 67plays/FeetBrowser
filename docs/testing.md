...
`test_h264.py` and `test_aac.py` are the two suites that carry binary
fixtures, and they have to. A codec can only be checked against streams
somebody else encoded and output somebody else's decoder produced; a fixture
generated by our own code would only prove that the decoder agrees with
itself. So `tests/fixtures/h264/` and `tests/fixtures/aac/` hold small real
streams and the exact output for each.

The two suites compare differently, because the two standards promise
differently. H.264 is bit-exact in the YUV domain, so every sample is
compared and one wrong byte fails. AAC defines its transform in real
arithmetic, so the PCM comparison is numerical -- the thresholds are the
measured error with a small margin, and the stages that *can* be compared
exactly (frame bit lengths, dequantisation, the IMDCT against its own
definition, the windows against their closed forms) are compared exactly so
that nothing hides behind the tolerance at the end.

A tolerance is not the only thing a fixture can hide behind. Code that no
vector reaches is not tested at any threshold, and that is not hypothetical:
the AAC decoder's temporal noise shaping filter could be deleted outright
without changing one sample of the vectors it originally shipped with,
because FFmpeg's encoder decides against TNS on almost everything. So
`test_aac.py` carries two tests whose job is the *fixtures* rather than the
decoder: they assert that the stereo tools, the noise substitution, the TNS
filter and all eight scalefactor band layouts are still being reached, and
they are what has to be re-read before any fixture is regenerated.
`test_h264.py` has no equivalent yet and wants one for the same reason --
being bit-exact does not help with a branch no stream takes.

Both skip cleanly with no `gfortran`, and because that state is hard to find
on purpose (all three CI platforms turned out to ship one) each has a test
that forces it on a machine that has one.
