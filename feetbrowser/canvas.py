...
class Font:
    ...
    def draw(self, surface, text, x, baseline, fill, scale=1.0):
        ...
        `scale` is device pixels per CSS pixel, and `x`/`baseline` arrive
        already multiplied by it. The pen advances by `advance * scale`, so
        the run occupies exactly the width layout measured -- and the glyph is
        *rasterised* at `self.size * scale` rather than magnified from a 1x
        bitmap, which is the whole point of the parameter: a 12px glyph blown
        up to 24 device pixels is the blur we are trying to remove, where the
        same outline filled at 24 has its curves resolved at the size it is
        shown. Coverage caching still works, because the key includes the
        size.
        """
        pen = float(x)
        widths = self._widths
    ...
    def _paint(self, surface, item):
        ...
        if kind == "rectangle":
            ...
            if photo is not None:
                # An image is the one thing here whose pixels are its own
                # rather than ours: a 100x100 photo is 100x100 CSS pixels,
                # which is 200x200 device pixels at 2x, so the blit
                # magnifies it. The real fix for a photo is to decode it at
                # the size it will be drawn; this at least puts it in the
