...
256-entry translate table per channel, and, where a raw span kernel was real
machine code, a call into it per row. Both are gone from this path, because
they were solving the problem the Rust fill solves and the Rust fill does the
whole rectangle in one crossing rather than one per row. Worth knowing that
the two paths did not round identically: the assembly took
`(src*a + dst*(255-a)) >> 8`, the tables and this code take `// 255`, so a
channel could land one level darker at the top of the range on Linux/x86-64
and only there.
"""
