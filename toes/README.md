...
- **sock-detective** — devtools, foot themed and hard-boiled. The "sock"
  ...
  `/style`, `/js`, `/cases`, `/errors`, `/help`) are full case files on the
  page's guts, including the JavaScript console output, all rendered through
  the normal pipeline (demonstrates `on_draw`, `on_motion`, `on_click`,
  `on_keypress`, `handle`).
