...
def test_js_report_shows_console():
    stub, tab = new_tab(
        "data:text/html,<script>console.log('hello from js');"
        "</script><p>fine</p>")
    tab.load("toe://sock/js")
    assert "THE SCRIPTURES" in display_text(tab)
    assert "hello from js" in display_text(tab)


def test_js_report_empty():
    stub, tab = new_tab("data:text/html,<p>no js here</p>")
    tab.load("toe://sock/js")
    assert "THE SCRIPTURES" in display_text(tab)
