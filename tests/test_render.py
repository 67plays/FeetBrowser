...
# display that puts two or three real pixels in each of those is the canvas's
# problem and the platform backends', and nowhere else's -- so this is where
# the arithmetic between them is checked, with no display attached. The half
...
def test_the_scale_is_one_unless_something_says_otherwise():
    ...
    with _ScaleEnv(None):
        ...
        # Bounded at both ends: a typo must not allocate a buffer measured
        # in gigabytes, or one measured in nothing.
...
def test_the_buffer_is_allocated_in_device_pixels():
    """The bug this section exists for: a 2x display was handed a 1x buffer
    and the window server stretched it, so every glyph and every form control
    was three-quarters interpolation. The page above stays in CSS pixels
    throughout, including across a move to a display of another density.
    """
    with _ScaleEnv(None):
        w = Window(width=300, height=200, scale=2.0)
        ...
        w.canvas = c
        assert c.device_size() == (600, 400), c.device_size()
        s = c.render()
        assert (s.width, s.height) == (600, 400), "surface is not device-sized"
        assert (c.winfo_width(), c.winfo_height()) == (300, 200)
        assert not w.set_scale(2.0), "the same scale twice is not a change"
        assert w.set_scale(1.0), "a real change must report itself"
        ...
        # number. Re-deriving it through a fractional scale can miss by one,
        # and a buffer short of the window leaves an unpainted seam.
        w.set_scale(1.5)
...
def test_drawing_lands_on_the_device_pixels_it_should():
    """Including line widths: a 1px border is 2 device pixels at 2x and 1 at
    1.25, but never 0, which would delete a border the page asked for."""
    ...
    assert canvasmod.Canvas(width=4, height=4, scale=1.25)._stroke(1) == 1
    assert canvasmod.Canvas(width=4, height=4, scale=0.5)._stroke(1) == 1, \
        "a border must not round away to nothing"
...
def test_a_click_at_a_device_pixel_maps_back_to_one_css_pixel():
    ...
    doing neither: the page would be sharp and every link in the wrong
    place. Windows offers 125% and 150%, so nothing here may assume the
    factor is a whole number.
    """
    with _ScaleEnv(None):
        ...
        # than merely close. At 1.5 the runs are uneven, which is the case
        # that catches an implementation that divides and rounds.
        w.set_scale(1.5)
...
def test_an_image_covers_the_device_pixels_it_should():
    """An image's pixels are its own, so a 2x2 picture in 2 CSS pixels has to
    be resampled up to the 4 device pixels it now covers rather than drawn at
    its own size in the corner -- and on the same source pixels a CSS resize
    would have picked, or the same image looks different depending on which
    of the two made it bigger.
    """
    ...
    # separate number from the source size, and a source buffer shorter than
    # it claims must not be read past.
    dense.blit_rgba(bytes([255, 255, 255, 255] * 4), 2, 2, 6, 6, True, 10, 10)
    assert _pixel(dense, 7, 7) == (255, 255, 255), "nothing was drawn"
    dense.blit_rgba(bytes([255, 0, 0, 255]), 4, 4, 0, 0, True, 8, 8)
